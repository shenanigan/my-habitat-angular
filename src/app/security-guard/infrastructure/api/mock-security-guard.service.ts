...
export class MockSecurityGuardService extends BaseService implements ISecurityGuardService {
    ...
    getSecurityGuard(): Observable<SecurityGuard> {
        return new Observable(subscriber => {
            ...
            securityGuard.households?.push(householdWife)
            securityGuard.households?.push(householdKid)
            securityGuard.households?.push(householdKid2)
            securityGuard.households?.push(householdMaid)
            securityGuard.households?.push(householdHelp)
            securityGuard.households?.push(householdVisitor1)
            securityGuard.households?.push(householdVisitor2)
